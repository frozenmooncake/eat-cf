...
- `snack`：小吃街，仅一楼
- 首页与菜单页楼层筛选项随所选区域动态显示：所选区域不含一餐厅时不显示二楼
...
- 排行榜窗口榜与菜品榜支持按区域多选筛选，默认选中校内餐厅（一餐厅、二餐厅），小吃街默认不选
- 排行榜窗口榜：筛选区域未全部有评分时最多显示 36 条；全部有评分后最多显示 50 条
- 排行榜综合分 = 总票数 × 60% + 众数星级 × 40%，按综合分降序排列；前端与 Worker 使用同一规则
